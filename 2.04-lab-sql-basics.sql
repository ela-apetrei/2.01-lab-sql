...
SELECT client_id, district_id FROM client WHERE district_id = 1 ORDER BY client_id ASC LIMIT 5;
...
SELECT loan_id, amount FROM loan ORDER BY amount DESC LIMIT 1;
...
SELECT  account_id, order_id FROM `order` WHERE account_id = 34;
...
SELECT DISTINCT(district_id), COUNT(client_id) FROM `client` WHERE district_id < 10 GROUP BY district_id ORDER BY district_id ASC; 

-- 14. In the card table, how many cards exist for each type? Rank the result starting with the most frequent type.
SELECT `type`, count(`type`) AS 'Count of each type' FROM bank.card GROUP BY `type` ORDER BY count(`type`) DESC;
...
SELECT `date`, COUNT(loan_id) AS 'Count of loans per day' FROM bank.loan WHERE `date` < 930907 GROUP BY `date` ORDER BY `date` DESC;
...
SELECT `date`, duration, COUNT(loan_id) AS 'Count of Loans' FROM bank.loan WHERE `date` BETWEEN 971200 AND 980101 GROUP BY `date` ORDER BY `DATE`, duration ASC;
...
SELECT account_id, prijem, vydaj,
